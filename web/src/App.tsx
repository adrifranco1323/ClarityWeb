...
export default function App() {
  ...
  const savePool = async (pool: Pool) => { setPools(current => [...current, pool]); await setDoc(doc(db, 'pools', pool.id), pool) }
  const updatePool = async (pool: Pool) => { setPools(current => current.map(item => item.id === pool.id ? pool : item)); await setDoc(doc(db, 'pools', pool.id), pool) }
